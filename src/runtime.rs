...
enum Value { Str(String), Num(i64), Bool(bool), Unit }
impl Value {
    fn truthy(&self) -> bool { match self { Value::Bool(b) => *b, Value::Num(n) => *n != 0, Value::Str(s) => !s.is_empty(), Value::Unit => false } }
}
...
pub fn run(program: &[Stmt]) -> Result<(), String> {
    ...
    for stmt in program {
        if let Stmt::Fn { name, params, body } = stmt {
            if funcs.insert(name.clone(), (params.clone(), body.clone())).is_some() { return Err(format!("duplicate function: {name}")); }
        }
    }
    let Some((params, body)) = funcs.get("main").cloned() else { return Err("entry point 'main' not found".into()) };
...
fn eval(expr: &Expr, env: &HashMap<String, Value>, funcs: &Functions) -> Result<Value, String> {
    match expr {
        ...
        Expr::Add(a, b) => {
            ...
            match (x, y) { (Value::Num(a), Value::Num(b)) => a.checked_add(b).map(Value::Num).ok_or("integer overflow".into()), (Value::Str(a), Value::Str(b)) => Ok(Value::Str(a + &b)), (a, b) => Err(format!("cannot add {a:?} and {b:?}")) }
        }
        ...
        Expr::Div(a, b) => { let y = num(b, env, funcs)?; if y == 0 { return Err("division by zero".into()) } Ok(Value::Num(num(a, env, funcs)?.checked_div(y).ok_or("integer overflow")?)) }
...
fn num(expr: &Expr, env: &HashMap<String, Value>, funcs: &Functions) -> Result<i64, String> { match eval(expr, env, funcs)? { Value::Num(n) => Ok(n), value => Err(format!("expected number, got {value:?}")) } }
...
fn display(value: &Value) -> String { match value { Value::Str(s) => s.clone(), Value::Num(n) => n.to_string(), Value::Bool(b) => b.to_string(), Value::Unit => "()".into() } }
